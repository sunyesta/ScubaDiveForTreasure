...
function MovementController.GameStart()
	...
	PlayerUtils.ObserveCharacterAdded(Player, function(character, characterTrove)
		if MovementController._CurrentMovementMode:Get() == MovementModes.Moving3D then
			...
			MovementController._Moving2D(World2DUtils.DefaultPlaneOrigin, World2DUtils.DefaultPlaneNormal)
		end

		WaterController.PlayerInWater:Observe(function(playerInWater)
...
function MovementController._Moving2D(planeOrigin: Vector3, planeNormal: Vector3)
	...
	-- FIX 1: Safely wait for the Character and essential parts to load in.
	-- Because Player spawns take a few frames to assemble, FindFirstChild would
	-- often return nil and abort the function before giving you any swimming logic!
	local character = Player.Character or Player.CharacterAdded:Wait()
	...
	if not humanoid or not rootPart then
		warn("MovementController: Humanoid or RootPart failed to load.")
		return
	end
	...
	-- FIX 2: Also wait for the Animator, as it's generated by the core scripts slightly after the Humanoid.
	...
	if animator and swimmingAnimation then
		...
		-- FIX 3: Force Priority to Action to ensure the default Roblox Animate script
		-- doesn't layer "Idle" or "Run" animations over your swimming animation.
	...
	movementTrove:Add(WaterController.PlayerInWater.Changed:Connect(function(inWater)
		if inWater then
			...
			-- Ensure Roblox transitions to the proper internal physics state
	...
	local updateConnection = RunService.RenderStepped:Connect(function(dt)
		...
		-- FIX 4: Enforce your custom states every frame!
		-- If the player spawns directly into the water, Roblox's internal engine will aggressively
		-- try to force them back into a "Running" state for the first few frames, disabling PlatformStand.
